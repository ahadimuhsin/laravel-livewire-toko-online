<div class="mb-5">
                ...
                <a href="/category/{{ $category->slug }}" class="text-decoration-none text-dark">
                    <div class="card h-100 border-0 shadow p-2 rounded-lg">
                        <div class="card-img">
                            ...
                            class="w-50">
                            <div class="title-category mt-2 font-weight-bold">
                                {{ $category->name }}
                            </div>
                        </div>
                    </div>
    ...
    <div class="container-fluid mt-3">
            ...
            <div class="col-6 col-md-3 mb-4">
                <div class="card h-100 border-0 shadow rounded-md">
                    ...
                    <div class="card-body">
                        ...
                        <div class="satuan" style="color: #999">
                            {{ $product->unit_weight }} {{ $product->unit }}
                        </div>

                        @if($product->discount > 0)
                        <div class="discount mt-2" style="#999">
                            <s>{{ money_id($product->price) }}</s>
                            <span style="background-color: #f6c07" class="badge badge-pill badge-warning text-white">
                            Save {{ $product->discount }} %
                            </span>
                        </div>
                        @endif

                        <div class="price font-wight-bold mt-3" style="color:#47b04b; font-size:20px">
                            {{ money_id($harga_diskon) }}
                        </div>
                        <button wire:click="addToCart({{ $product->id }})"
                            class="btn btn-success btn-md mt-3 btn-block shadow-md">
                            Beli
                        </button>
                    </div>
                </div>
        ...
        <div class="row justify-content-center mt-5">
            ...
            <button wire:click="loadMore()" class="btn btn-dark btn-lg shadow-sm">
                Load More
            </button>
            @endif
        </div>
